...
import { setCatalogItems, showItem } from '../toolkit/toolkitSlice';
import Search from "./Search";

const Catalog = (form) => {
    const renderItemsNumber = 6;
    ...
    useEffect(() => {
      ...
      if (storedCategories) {
        setCategoriesArr(JSON.parse(storedCategories));
      } else {
        loadCategories();
      }
    }, []);
    ...
    const loadCategories = () => {
      ...
      const loadCatalogItems = () => {
        ...
        fetchDataPromise
          .then(data => {
            setmoreItems(data.length % renderItemsNumber === 0 ? true : false);
